...
> **Note:** Data transformation (`map`, `filter`, `fold`, `find`, `collect`) and resilience (`retry`, `validate`) are stdlib methods, not compiler patterns. See [Built-in Functions](/docs/spec/11-built-in-functions).
...
See [Errors and Panics § Catching Panics](/docs/spec/20-errors-and-panics#catching-panics).
