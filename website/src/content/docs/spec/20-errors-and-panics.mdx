...
Recoverable errors use `Result<T, E>` and `Option<T>` types. See [Types](/docs/spec/06-types) for type definitions and methods.

### Error Propagation

The `?` operator propagates errors. See [Control Flow](/docs/spec/19-control-flow) for details.
