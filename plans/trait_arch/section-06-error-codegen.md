...
- [ ] Add completeness test for `.md` doc files
- [ ] Add completeness test for descriptions
- [ ] Update rendering to use descriptive variant names (optional — can be done incrementally)
...
- [ ] Write the macro
- [ ] Replace enum with invocation
- [ ] Rename variants (or add aliases)
- [ ] Update all match sites
- [ ] `./test-all.sh` passes
- [ ] Add completeness tests
- [ ] Delete redundant manual code

---

## 06.5 Completion Checklist

- [ ] `define_error_codes!` macro defined and invoked
- [ ] All 38+ error codes declared in single macro invocation
- [ ] `from_u16()`, `to_u16()`, `name()`, `description()` generated by macro
- [ ] `ErrorCode::ALL` and `COUNT` available for iteration
- [ ] Variant names are descriptive (not just `E{XXXX}`)
- [ ] Completeness test validates each code has a `.md` doc file
- [ ] Completeness test validates each code has a description
- [ ] All rendering match sites use descriptive variant names
- [ ] Manual `from_u16()`/`to_u16()` match arms deleted
