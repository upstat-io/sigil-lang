...
use sigil_ir::{Name, Span, Token, TokenKind, TokenList, StringInterner};
...
mod tests {
    ...
    fn test_cursor_navigation() {
        ...
        let tokens = sigil_lexer::lex("let x = 42", &interner);
    ...
    fn test_expect_success() {
        let interner = StringInterner::new();
        let tokens = sigil_lexer::lex("let x", &interner);
        let tokens = Box::leak(Box::new(tokens));
        let interner = Box::leak(Box::new(interner));
        let mut cursor = Cursor::new(tokens, interner);
    ...
    fn test_skip_newlines() {
        let interner = StringInterner::new();
        let tokens = sigil_lexer::lex("let\n\n\nx", &interner);
        let tokens = Box::leak(Box::new(tokens));
        let interner = Box::leak(Box::new(interner));
        let mut cursor = Cursor::new(tokens, interner);
    ...
    fn test_lookahead() {
        let interner = StringInterner::new();
        let tokens = sigil_lexer::lex("foo()", &interner);
        let tokens = Box::leak(Box::new(tokens));
        let interner = Box::leak(Box::new(interner));
    ...
    fn test_check_type_keyword() {
        let interner = StringInterner::new();
        let tokens = sigil_lexer::lex("int float bool str", &interner);
        let tokens = Box::leak(Box::new(tokens));
        let interner = Box::leak(Box::new(interner));
        let mut cursor = Cursor::new(tokens, interner);
