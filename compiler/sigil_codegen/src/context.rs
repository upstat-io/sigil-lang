...
use sigil_ir::{ExprId, Name, StringInterner, TypeId};
...
///
/// Holds all state needed to generate C code from a typed Sigil module.
...
impl<'a> CodegenContext<'a> {
    ...
    /// Mangle a Sigil name for C compatibility.
    ///
...
mod tests {
    ...
    fn test_mangle_simple() {
        ...
        assert_eq!(ctx.mangle(name), "sigil_foo");
    }
    ...
    fn test_mangle_special_chars() {
        ...
        let ctx = CodegenContext::new(&interner, &type_interner, &[]);
        assert_eq!(ctx.mangle(name), "sigil_my_func");
    }
