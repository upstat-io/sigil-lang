...
//!
//! Generates C code for Sigil function bodies.

use sigil_ir::{ExprArena, ExprId, ast::{ExprKind, FunctionSeq, SeqBinding, BindingPattern}};
...
pub fn emit_cleanup(
    ctx: &mut CodegenContext<'_>,
    locals: &[sigil_ir::Name],
) {
    for &name in locals {
        if ctx.needs_release(name) {
            let var_name = ctx.mangle(name);
            ctx.writeln(&format!("sigil_arc_release({var_name});"));
        }
    }
}
