...
use sigil_ir::{ast::{ExprKind, FunctionSeq, SeqBinding}, ExprArena, ExprId, Name, TypeId};
    ...
    fn visit_expr(&mut self, id: ExprId, context: Ownership) {
        ...
        match &expr.kind {
            ...
            ExprKind::Let { pattern, init, .. } => {
                ...
                if let sigil_ir::ast::BindingPattern::Name(name) = pattern {
                    if self.needs_arc(type_id) {
            ...
            ExprKind::Block { stmts, result } => {
                for stmt in self.arena.get_stmt_range(*stmts) {
                    let expr_id = match &stmt.kind {
                        sigil_ir::StmtKind::Expr(id) => Some(*id),
                        sigil_ir::StmtKind::Let { init, .. } => Some(*init),
                    };
