...
//!
//! Generates C code for Ori statements.
...
fn emit_let_binding_from_stmt(
    ...
    match pattern {
        ...
        BindingPattern::List { elements, rest } => {
            // Destructure list
            let tmp = ctx.fresh_temp();
            ctx.writeln(&format!("ori_list_t {tmp} = {init_expr};"));

            for (i, pat) in elements.iter().enumerate() {
                if let BindingPattern::Name(name) = pat {
                    let var_name = ctx.mangle(*name);
                    ctx.writeln(&format!(
                        "__auto_type {var_name} = ori_list_get({tmp}, {i});"
                    ));
                }
            }
            ...
            if let Some(rest_name) = rest {
                ...
                ctx.writeln(&format!(
                    "ori_list_t {var_name} = ori_list_slice({tmp}, {start}, -1);"
                ));
            }
        }
