...
//!
//! Generates C code from typed Ori modules with performance optimizations:
...
/// C code generator.
///
/// Generates C code from a typed Ori module.
...
impl<'a> CCodegen<'a> {
    ...
    pub fn analyze_ownership(&mut self, root: ori_ir::ExprId) {
        let analysis = OwnershipAnalysis::new(self.arena, self.type_interner, self.expr_types);
        self.ownership = analysis.analyze(root);
    }

    /// Generate C code for a module.
    pub fn generate(&self, module: &Module) -> CodegenResult {
        let mut ctx = CodegenContext::new(self.interner, self.type_interner, self.expr_types)
        ...
        if let Some(main_func) = module
            .functions
            .iter()
            .find(|f| self.interner.lookup(f.name) == "main")
        {
    ...
    #[expect(
        clippy::unused_self,
        reason = "method signature for consistency with other emit methods"
    )]
    fn emit_header(&self, ctx: &mut CodegenContext<'_>) {
        ctx.writeln("// Generated by Ori Compiler");
    ...
    fn map_return_type(&self, func: &ori_ir::ast::Function) -> String {
    ...
    fn emit_function(&self, ctx: &mut CodegenContext<'_>, func: &ori_ir::ast::Function) {
        let name = ctx.mangle(func.name);
        let params = self.arena.get_params(func.params);
    ...
    /// Emit the `main()` wrapper that calls the Ori @main function.
    fn emit_main_wrapper(&self, ctx: &mut CodegenContext<'_>, func: &ori_ir::ast::Function) {
        let ori_main = ctx.mangle(func.name);
        let ret_type = self.map_return_type(func);
        ...
        if ret_type == "int64_t" || ret_type == "int" {
            ...
            ctx.write(&ori_main);
            ctx.write("();\n");
            ctx.writeln("ori_runtime_cleanup();");
...
mod tests {
    ...
    fn test_codegen_empty_module() {
        ...
        assert!(result.code.contains("ori_string_t"));
    }
}
