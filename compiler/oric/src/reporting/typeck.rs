...
impl<'a> TypeErrorRenderer<'a> {
    ...
    ///
    /// Mirrors `TypeCheckError::message()` but uses `Pool::format_type()`
    /// for complete type rendering.
        ...
        match &error.kind {
            TypeErrorKind::Mismatch {
                ...
                for problem in problems {
                    if let Some(detail) = self.problem_message(problem, *expected, *found) {
            ...
            TypeErrorKind::UnknownIdent { name, .. } => {
                format!("unknown identifier `{}`", self.format_name(*name))
            }
    ...
    /// Generate a specific message for a `TypeProblem` with full type names.
    fn problem_message(
        &self,
        problem: &TypeProblem,
        _expected: Idx,
