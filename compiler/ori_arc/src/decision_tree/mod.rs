...
//! # Architecture
//!
//! The decision tree lives entirely in `ori_arc` (no LLVM dependency). The
//! `ori_llvm` codegen layer mechanically translates the resulting ARC IR
//! terminators (`Switch` → LLVM `switch`, `Branch` → LLVM `br`).
//!
//! # References
//!
...
///
/// The tree structure follows Maranget (2008), as implemented by Roc
/// and Elm.
...
// ── Pattern Row (for the matrix algorithm) ──────────────────────────
...
impl FlatPattern {
    ...
    fn collect_bindings(&self, path: &ScrutineePath, out: &mut Vec<(Name, ScrutineePath)>) {
        match self {
