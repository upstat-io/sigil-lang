...
fn bench_parser_nested_arithmetic(c: &mut Criterion) {
    ...
    c.bench_function("parser/nested_arithmetic", |b| {
        b.iter(|| {
            let file = SourceFile::new(&db, PathBuf::from("/bench.si"), NESTED_ARITHMETIC.to_string());
            black_box(parsed(&db, file))
        })
    });
}
...
fn bench_parser_conditional(c: &mut Criterion) {
    ...
    c.bench_function("parser/conditional", |b| {
        b.iter(|| {
            let file = SourceFile::new(&db, PathBuf::from("/bench.si"), CONDITIONAL.to_string());
            black_box(parsed(&db, file))
        })
    });
}
...
fn bench_parser_list(c: &mut Criterion) {
    ...
    c.bench_function("parser/list_literal", |b| {
        b.iter(|| {
            let file = SourceFile::new(&db, PathBuf::from("/bench.si"), LIST_LITERAL.to_string());
            black_box(parsed(&db, file))
        })
    });
}
...
fn bench_parser_patterns(c: &mut Criterion) {
    ...
    group.bench_function("map", |b| {
        b.iter(|| {
            let file = SourceFile::new(&db, PathBuf::from("/bench.si"), PATTERN_MAP.to_string());
            black_box(parsed(&db, file))
        })
    });
    ...
    group.bench_function("fold", |b| {
        b.iter(|| {
            let file = SourceFile::new(&db, PathBuf::from("/bench.si"), PATTERN_FOLD.to_string());
            black_box(parsed(&db, file))
        })
    });
