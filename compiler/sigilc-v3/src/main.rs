//! Sigil V3 Compiler CLI
//!
//! Salsa-first incremental compiler.

use sigilc_v3::{CompilerDb, SourceFile, Db};
use sigilc_v3::query::{tokens, parsed, typed, evaluated};
use sigilc_v3::test::{TestRunner, TestRunnerConfig, TestSummary};
...
fn main() {
    ...
    match command.as_str() {
        "run" => {
            if args.len() < 3 {
                eprintln!("Usage: sigil-v3 run <file.si>");
                std::process::exit(1);
            }
            run_file(&args[2]);
        }
        ...
        "check" => {
            if args.len() < 3 {
                eprintln!("Usage: sigil-v3 check <file.si>");
                std::process::exit(1);
            }
            check_file(&args[2]);
        }
        "parse" => {
            if args.len() < 3 {
                eprintln!("Usage: sigil-v3 parse <file.si>");
                std::process::exit(1);
            }
            parse_file(&args[2]);
        }
        "lex" => {
            if args.len() < 3 {
                eprintln!("Usage: sigil-v3 lex <file.si>");
                std::process::exit(1);
            }
            lex_file(&args[2]);
        }
        ...
        "version" | "--version" | "-v" => {
            println!("Sigil V3 Compiler 0.1.0-alpha.1");
...
fn print_usage() {
    println!("Sigil V3 Compiler (Salsa-first incremental compilation)");
    println!();
    println!("Usage: sigil-v3 <command> [options]");
    println!();
    println!("Commands:");
    ...
    println!();
    println!("Examples:");
    println!("  sigil-v3 run main.si");
    println!("  sigil-v3 test                    # Run all spec tests");
    println!("  sigil-v3 test tests/spec/patterns/");
    println!("  sigil-v3 test --filter=map");
    println!("  sigil-v3 check lib.si");
    println!("  sigil-v3 main.si       (shorthand for 'run')");
}
...
fn check_file(path: &str) {
    ...
    let mut tested_functions: std::collections::HashSet<sigilc_v3::Name> = std::collections::HashSet::new();
...
fn print_coverage_report(report: &sigilc_v3::test::CoverageReport) {
    ...
    for file in &summary.files {
        ...
        for result in &file.results {
            let status = match &result.outcome {
                ...
                sigilc_v3::TestOutcome::Failed(msg) => {
                    format!("  FAIL: {} - {}", result.name, msg)
                }
                sigilc_v3::TestOutcome::Skipped(reason) => {
                    if verbose {
