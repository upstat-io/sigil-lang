...
impl EvalOutput {
    ...
    pub fn from_value(value: &Value, interner: &StringInterner) -> Self {
        match value {
            ...
            Value::Builtin(_, name) => EvalOutput::Function(format!("<builtin {}>", name)),
            Value::Struct(s) => {
...
mod tests {
    ...
    fn test_builtin_assert() {
        ...
        assert!(builtin_assert(&[Value::Bool(false)]).is_err());
        assert!(builtin_assert(&[]).is_err());
    }

    #[test]
    fn test_builtin_assert_eq() {
        assert!(builtin_assert_eq(&[Value::Int(1), Value::Int(1)]).is_ok());
        assert!(builtin_assert_eq(&[Value::Int(1), Value::Int(2)]).is_err());
    }
    ...
    fn test_builtin_float() {
        assert_eq!(builtin_float(&[Value::Int(3)]).unwrap(), Value::Float(3.0));
    }

    #[test]
    fn test_builtin_compare() {
        assert_eq!(builtin_compare(&[Value::Int(1), Value::Int(2)]).unwrap(), Value::Int(-1));
        assert_eq!(builtin_compare(&[Value::Int(2), Value::Int(1)]).unwrap(), Value::Int(1));
        assert_eq!(builtin_compare(&[Value::Int(1), Value::Int(1)]).unwrap(), Value::Int(0));
    }
