...
///
/// A separate struct to hold both the function pointer and display name,
/// making the enum variant simpler.
#[derive(Clone)]
pub struct Builtin {
    /// The function implementation.
    pub func: fn(&[Value]) -> Result<Value, String>,
    /// Display name for debugging/printing.
    pub name: &'static str,
}

impl Builtin {
    /// Create a new builtin.
    pub fn new(func: fn(&[Value]) -> Result<Value, String>, name: &'static str) -> Self {
        Builtin { func, name }
    }
...
impl PartialEq for Builtin {
    fn eq(&self, other: &Self) -> bool {
        // Compare by name since function pointers aren't directly comparable
