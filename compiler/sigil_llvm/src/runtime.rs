//! Runtime functions for LLVM-compiled Sigil code.
//!
//! These functions are called by LLVM-compiled code at runtime.
//! They're declared as `extern "C"` so LLVM can link to them.
//!
//! # Safety
//!
//! This module uses `#[no_mangle]` for FFI compatibility with LLVM JIT.
//! All functions are safe Rust but need stable symbol names.

#![allow(unsafe_code)]

use std::ffi::CStr;
use std::sync::atomic::{AtomicBool, Ordering};

/// Global flag to track if a panic occurred (for test assertions).
static PANIC_OCCURRED: AtomicBool = AtomicBool::new(false);

/// Global storage for panic message.
static mut PANIC_MESSAGE: Option<String> = None;

/// Sigil string representation: { i64 len, *const u8 data }
#[repr(C)]
pub struct SigilStr {
    pub len: i64,
    pub data: *const u8,
}

impl SigilStr {
    /// Convert to Rust string slice.
    ///
    /// # Safety
    /// Caller must ensure data pointer is valid and len is correct.
    #[allow(unsafe_code)]
    pub unsafe fn as_str(&self) -> &str {
        if self.data.is_null() || self.len <= 0 {
            return "";
        }
        let slice = std::slice::from_raw_parts(self.data, self.len as usize);
        std::str::from_utf8_unchecked(slice)
    }
}

/// Sigil list representation: { i64 len, i64 cap, *mut u8 data }
#[repr(C)]
pub struct SigilList {
    pub len: i64,
    pub cap: i64,
    pub data: *mut u8,
}

/// Sigil Option representation: { i8 tag, T value }
/// tag = 0: None, tag = 1: Some
#[repr(C)]
pub struct SigilOption<T> {
    pub tag: i8,
    pub value: T,
}

/// Sigil Result representation: { i8 tag, T value }
/// tag = 0: Ok, tag = 1: Err
#[repr(C)]
pub struct SigilResult<T> {
    pub tag: i8,
    pub value: T,
}

/// Print a string to stdout.
///
/// # Safety
/// This function is called from LLVM-compiled code.
#[no_mangle]
pub extern "C" fn sigil_print(s: *const SigilStr) {
    if s.is_null() {
        println!();
        return;
    }

    // SAFETY: Caller ensures s points to a valid SigilStr
    let sigil_str = unsafe { &*s };
    let text = unsafe { sigil_str.as_str() };
    println!("{text}");
}

/// Print an integer to stdout.
#[no_mangle]
pub extern "C" fn sigil_print_int(n: i64) {
    println!("{n}");
}

/// Print a float to stdout.
#[no_mangle]
pub extern "C" fn sigil_print_float(f: f64) {
    println!("{f}");
}

/// Print a boolean to stdout.
#[no_mangle]
pub extern "C" fn sigil_print_bool(b: bool) {
    println!("{b}");
}

/// Panic with a message.
///
/// # Safety
/// This function is called from LLVM-compiled code.
#[no_mangle]
pub extern "C" fn sigil_panic(s: *const SigilStr) {
    PANIC_OCCURRED.store(true, Ordering::SeqCst);

    let msg = if s.is_null() {
        "panic!".to_string()
    } else {
        // SAFETY: Caller ensures s points to a valid SigilStr
        let sigil_str = unsafe { &*s };
        let text = unsafe { sigil_str.as_str() };
        text.to_string()
    };

    // Store message for later retrieval
    // SAFETY: Single-threaded test execution
    unsafe {
        PANIC_MESSAGE = Some(msg.clone());
    }

    // Don't actually panic - just set the flag
    // This allows tests to check for expected panics
    eprintln!("sigil panic: {msg}");
}

/// Panic with a C string message.
///
/// # Safety
/// This function is called from LLVM-compiled code.
#[no_mangle]
pub extern "C" fn sigil_panic_cstr(s: *const i8) {
    PANIC_OCCURRED.store(true, Ordering::SeqCst);

    let msg = if s.is_null() {
        "panic!".to_string()
    } else {
        // SAFETY: Caller ensures s points to a valid C string
        let cstr = unsafe { CStr::from_ptr(s) };
        cstr.to_string_lossy().to_string()
    };

    // SAFETY: Single-threaded test execution
    unsafe {
        PANIC_MESSAGE = Some(msg.clone());
    }

    eprintln!("sigil panic: {msg}");
}

/// Check if a panic occurred (for test assertions).
pub fn did_panic() -> bool {
    PANIC_OCCURRED.load(Ordering::SeqCst)
}

/// Get the panic message if one occurred.
///
/// # Safety
/// Must be called from the same thread that ran the test.

pub fn get_panic_message() -> Option<String> {
    // SAFETY: Single-threaded test execution
    unsafe { PANIC_MESSAGE.clone() }
}

/// Reset panic state (call before each test).
///
/// # Safety
/// Must be called from the same thread that will run the test.

pub fn reset_panic_state() {
    PANIC_OCCURRED.store(false, Ordering::SeqCst);
    // SAFETY: Single-threaded test execution
    unsafe {
        PANIC_MESSAGE = None;
    }
}

/// Assert that a condition is true.
#[no_mangle]
pub extern "C" fn sigil_assert(condition: bool) {
    if !condition {
        sigil_panic_cstr(b"assertion failed\0".as_ptr().cast());
    }
}

/// Assert that two integers are equal.
#[no_mangle]
pub extern "C" fn sigil_assert_eq_int(actual: i64, expected: i64) {
    if actual != expected {
        eprintln!("assertion failed: {actual} != {expected}");
        PANIC_OCCURRED.store(true, Ordering::SeqCst);
        // SAFETY: Single-threaded test execution
        unsafe {
            PANIC_MESSAGE = Some(format!("assertion failed: {actual} != {expected}"));
        }
    }
}

/// Assert that two booleans are equal.
#[no_mangle]
pub extern "C" fn sigil_assert_eq_bool(actual: bool, expected: bool) {
    if actual != expected {
        eprintln!("assertion failed: {actual} != {expected}");
        PANIC_OCCURRED.store(true, Ordering::SeqCst);
        unsafe {
            PANIC_MESSAGE = Some(format!("assertion failed: {actual} != {expected}"));
        }
    }
}

/// Allocate a new list with given capacity.
#[no_mangle]
pub extern "C" fn sigil_list_new(capacity: i64, elem_size: i64) -> *mut SigilList {
    let cap = capacity.max(0) as usize;
    let size = elem_size.max(1) as usize;

    // Allocate the list struct
    let list = Box::new(SigilList {
        len: 0,
        cap: cap as i64,
        data: if cap > 0 {
            // Allocate data buffer
            let layout = std::alloc::Layout::array::<u8>(cap * size)
                .unwrap_or_else(|_| std::alloc::Layout::new::<u8>());
            // SAFETY: Layout is valid
            unsafe { std::alloc::alloc(layout) }
        } else {
            std::ptr::null_mut()
        },
    });

    Box::into_raw(list)
}

/// Free a list.
#[no_mangle]
pub extern "C" fn sigil_list_free(list: *mut SigilList, elem_size: i64) {
    if list.is_null() {
        return;
    }

    // SAFETY: Caller ensures list is valid
    unsafe {
        let list = Box::from_raw(list);
        if !list.data.is_null() && list.cap > 0 {
            let size = elem_size.max(1) as usize;
            let layout = std::alloc::Layout::array::<u8>(list.cap as usize * size)
                .unwrap_or_else(|_| std::alloc::Layout::new::<u8>());
            std::alloc::dealloc(list.data, layout);
        }
    }
}

/// Get the length of a list.
#[no_mangle]
pub extern "C" fn sigil_list_len(list: *const SigilList) -> i64 {
    if list.is_null() {
        return 0;
    }
    // SAFETY: Caller ensures list is valid
    unsafe { (*list).len }
}

/// Compare two integers (for sorting, etc.)
#[no_mangle]
pub extern "C" fn sigil_compare_int(a: i64, b: i64) -> i32 {
    match a.cmp(&b) {
        std::cmp::Ordering::Less => -1,
        std::cmp::Ordering::Equal => 0,
        std::cmp::Ordering::Greater => 1,
    }
}

/// Get minimum of two integers.
#[no_mangle]
pub extern "C" fn sigil_min_int(a: i64, b: i64) -> i64 {
    a.min(b)
}

/// Get maximum of two integers.
#[no_mangle]
pub extern "C" fn sigil_max_int(a: i64, b: i64) -> i64 {
    a.max(b)
}

/// Concatenate two strings.
///
/// Returns a new SigilStr with the concatenated result.
/// The caller is responsible for freeing the result.
#[no_mangle]
pub extern "C" fn sigil_str_concat(a: *const SigilStr, b: *const SigilStr) -> SigilStr {
    let a_str = if a.is_null() {
        ""
    } else {
        unsafe { (*a).as_str() }
    };
    let b_str = if b.is_null() {
        ""
    } else {
        unsafe { (*b).as_str() }
    };

    let result = format!("{a_str}{b_str}");
    let len = result.len() as i64;
    let data = result.into_boxed_str();
    let ptr = Box::into_raw(data) as *const u8;

    SigilStr { len, data: ptr }
}

/// Compare two strings for equality.
#[no_mangle]
pub extern "C" fn sigil_str_eq(a: *const SigilStr, b: *const SigilStr) -> bool {
    let a_str = if a.is_null() {
        ""
    } else {
        unsafe { (*a).as_str() }
    };
    let b_str = if b.is_null() {
        ""
    } else {
        unsafe { (*b).as_str() }
    };

    a_str == b_str
}

/// Compare two strings for inequality.
#[no_mangle]
pub extern "C" fn sigil_str_ne(a: *const SigilStr, b: *const SigilStr) -> bool {
    !sigil_str_eq(a, b)
}

/// Assert two strings are equal.
#[no_mangle]
pub extern "C" fn sigil_assert_eq_str(actual: *const SigilStr, expected: *const SigilStr) {
    let actual_str = if actual.is_null() {
        ""
    } else {
        unsafe { (*actual).as_str() }
    };
    let expected_str = if expected.is_null() {
        ""
    } else {
        unsafe { (*expected).as_str() }
    };

    if actual_str != expected_str {
        eprintln!("assertion failed: \"{actual_str}\" != \"{expected_str}\"");
        PANIC_OCCURRED.store(true, Ordering::SeqCst);
        unsafe {
            PANIC_MESSAGE = Some(format!("assertion failed: \"{actual_str}\" != \"{expected_str}\""));
        }
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_sigil_print_int() {
        sigil_print_int(42);
    }

    #[test]
    fn test_sigil_assert_eq_int_pass() {
        reset_panic_state();
        sigil_assert_eq_int(42, 42);
        assert!(!did_panic());
    }

    #[test]
    fn test_sigil_assert_eq_int_fail() {
        reset_panic_state();
        sigil_assert_eq_int(42, 43);
        assert!(did_panic());
    }

    #[test]
    fn test_sigil_compare_int() {
        assert_eq!(sigil_compare_int(1, 2), -1);
        assert_eq!(sigil_compare_int(2, 2), 0);
        assert_eq!(sigil_compare_int(3, 2), 1);
    }

    #[test]
    fn test_sigil_min_max() {
        assert_eq!(sigil_min_int(3, 5), 3);
        assert_eq!(sigil_max_int(3, 5), 5);
    }
}
