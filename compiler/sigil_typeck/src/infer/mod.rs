...
pub fn add_pattern_bindings(pattern: &sigil_ir::BindingPattern, bound: &mut HashSet<Name>) {
    match pattern {
        sigil_ir::BindingPattern::Name(name) => {
            bound.insert(*name);
        }
        sigil_ir::BindingPattern::Struct { fields } => {
        ...
        sigil_ir::BindingPattern::Tuple(patterns) => {
            for p in patterns {
                add_pattern_bindings(p, bound);
            }
        }
        sigil_ir::BindingPattern::List { elements, rest } => {
            for p in elements {
                add_pattern_bindings(p, bound);
            }
        ...
        sigil_ir::BindingPattern::Wildcard => {}
    }
}
