...
use crate::ir::{TokenKind, TypeId};
use crate::parser::Parser;

impl<'a> Parser<'a> {
    /// Parse a type expression.
    /// Returns Some(TypeId) for primitive types, None for unknown/complex types.
    pub(in crate::parser) fn parse_type(&mut self) -> Option<TypeId> {
...
#[cfg(test)]
mod tests {
    use crate::ir::{StringInterner, TypeId};
    use crate::lexer;
    use crate::parser::Parser;
    ...
    fn parse_type_from_source(source: &str) -> Option<TypeId> {
        ...
        let tokens = lexer::lex(&full_source, &interner);
