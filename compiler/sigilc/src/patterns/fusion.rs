...
use crate::ir::{ExprId, Name, Span, FunctionExpKind};
...
impl FusedPattern {
    ...
    pub fn evaluate(&self, exec: &mut dyn PatternExecutor) -> EvalResult {
        match self {
            FusedPattern::MapFilter { input, map_fn, filter_fn } => {
                ...
                match items {
                    Value::List(list) => {
                        ...
                        for item in list.iter() {
                            ...
                            // Apply filter
                            if exec.call(filter_f.clone(), vec![mapped.clone()])?.is_truthy() {
                                results.push(mapped);
                            }
                        }
            ...
            FusedPattern::FilterMap { input, filter_fn, map_fn } => {
                let items = exec.eval(*input)?;
                let filter_f = exec.eval(*filter_fn)?;
                let map_f = exec.eval(*map_fn)?;
            ...
            FusedPattern::MapFold { input, map_fn, init, fold_fn } => {
                let items = exec.eval(*input)?;
                let map_f = exec.eval(*map_fn)?;
            ...
            FusedPattern::FilterFold { input, filter_fn, init, fold_fn } => {
                let items = exec.eval(*input)?;
                let filter_f = exec.eval(*filter_fn)?;
                let mut acc = exec.eval(*init)?;
                let fold_f = exec.eval(*fold_fn)?;
            ...
            FusedPattern::MapFind { input, map_fn, find_fn } => {
                let items = exec.eval(*input)?;
                let map_f = exec.eval(*map_fn)?;
            ...
            FusedPattern::FilterFind { input, filter_fn, find_fn } => {
                let items = exec.eval(*input)?;
                let filter_f = exec.eval(*filter_fn)?;
                let find_f = exec.eval(*find_fn)?;
