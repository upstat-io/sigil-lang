...
mod tests {
    ...
    fn test_sarif_emitter_basic() {
        ...
        let mut emitter = SarifEmitter::new(&mut output, "sigilc", "0.1.0")
            .with_artifact("src/main.si")
        ...
        assert!(text.contains("\"name\": \"sigilc\""));
    ...
    fn test_sarif_emitter_related_locations() {
        let mut output = Vec::new();
        let source = "let x = 10\nlet y = x + z";
        let mut emitter = SarifEmitter::new(&mut output, "sigilc", "0.1.0").with_source(source);
