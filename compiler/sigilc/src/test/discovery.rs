...
//! Convention: All .si files can contain tests (functions with `tests` keyword).
...
mod tests {
    ...
    fn test_discover_si_files() {
        ...
        File::create(dir.path().join("test1.si")).unwrap();
        File::create(dir.path().join("test2.si")).unwrap();
    ...
    fn test_discover_recursive() {
        ...
        File::create(dir.path().join("root.si")).unwrap();
        File::create(sub.join("nested.si")).unwrap();
    ...
    fn test_skip_hidden_and_target() {
        ...
        File::create(hidden.join("test.si")).unwrap();
        File::create(target.join("test.si")).unwrap();
        File::create(dir.path().join("real.si")).unwrap();

        let files = discover_tests(dir.path());
        assert_eq!(files.len(), 1);
        assert!(files[0].path.ends_with("real.si"));
    }

    #[test]
    fn test_discover_single_file() {
        let dir = tempdir().unwrap();
        let path = dir.path().join("test.si");
