...
```ori
@fetch_users_summary (ids: [int]) -> UsersSummary uses Http, Async, Logger = run(
...
@fetch_user_bundle (user_id: int) -> Result<DataBundle, Error> uses Http, Async = run(
...
@send_notifications (events: [Event]) -> void uses Http, Async, Logger =
    spawn(
        tasks: for event in events yield () -> run(
...
```ori
@fetch_with_fallback (url: str, fallback: str) -> str uses Http, Async = run(
    let result = timeout(
        op: Http.get(url: url),
        after: 5s,
    ),
...
```ori
@fetch_with_retries (url: str) -> Result<str, Error> uses Http, Async = run(
    let attempts = [1, 2, 3],

    for attempt in attempts do run(
        let result = timeout(
            op: Http.get(url: url),
            after: 5s,
        ),
...
```ori
@process_batch (items: [Item]) -> BatchResult uses Async, Logger = run(
    let results = nursery(
        body: n -> run(
...
```ori
@crawl_pages (start_url: str, max_depth: int) -> [Page] uses Http, Async = run(
    let visited: Set<str> = Set.new(),
    let pages: [Page] = [],

    nursery(
        body: n -> run(
            @crawl (url: str, depth: int) -> void = run(
...
```ori
@process_batches (batches: [[Item]]) -> [[Result<Output, Error>]] uses Async = run(
    nursery(
...
```ori
@fetch_with_summary (ids: [int]) -> FetchSummary uses Http, Async = run(
    let results = parallel(
...
@check_page (url: str) -> PageResult uses Http, Clock, Async = run(
    let start = Clock.now(),
...
@health_check (urls: [str]) -> HealthCheck uses Http, Clock, Async, Logger = run(
    ...
    let page_results = for r in results yield match(
        r,
        Ok(pr) -> pr,
        Err(e) -> PageResult {
            url: "unknown",
...
@monitor_continuously (urls: [str], interval: Duration) -> void
    uses Http, Clock, Async, Logger = run(
    nursery(
        body: n -> run(
            loop(
