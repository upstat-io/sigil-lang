...
Ori's REPL (Read-Eval-Print Loop) supports interactive exploration and validation for both humans and AI. It features JSON output mode, semantic addressing integration, and built-in test execution.
...
```bash
ori repl              # Interactive mode
ori repl --json       # JSON output mode
ori repl --load src/  # Load files before starting
```
...
| `:quit` / `:q` | Exit REPL | `:q` |
| `:type <expr>` | Show type of expression | `:type map([1, 2], item -> item * 2)` |
| `:ast <expr>` | Show AST (for debugging) | `:ast 1 + 2 * 3` |
| `:env` | Show current bindings | `:env` |
| `:clear` | Clear all bindings | `:clear` |
| `:load <file>` | Load and evaluate file | `:load src/utils.ori` |
...
- Line ends with operator (`+`, `&&`, `||`, etc.)

### Natural Continuation

Expressions naturally continue when a line ends with an operator:
...
> @quadruple (number: int) -> int = double(double(number))
...
```
> x = 42
> @helper (number: int) -> int = number + 1
> :save mysession.repl
...
```
> :load src/math.ori
Loaded src/math.ori: 5 functions, 2 types, 0 errors
```
...
```json
{
  "expression": "map([1, 2, 3], item -> item * 2)",
  "type": "[int]",
  "type_details": {
    "kind": "list",
    "element_type": "int"
...
Note: Change is temporary (session only). Use 'ori edit' for permanent changes.
```
...
History is saved to `~/.ori/repl_history` and persists across sessions.
...
```bash
ori repl --json           # Start in JSON mode
ori repl --load src/      # Load files on start
ori repl --no-history     # Don't save history
ori repl --quiet          # Suppress banner
```

### Environment Variables

```bash
ORI_REPL_HISTORY=~/.ori/history  # History file location
ORI_REPL_HISTSIZE=1000             # Max history entries
```
