...
Edit operations enable targeted modifications to Ori code using semantic addresses. Instead of regenerating entire files, AI makes precise changes to specific elements.
...
```json
{
  ...
  "address": "@helper",
  "references": [
    { "file": "src/main.ori", "line": 23, "expression": "helper(value)" },
    { "file": "src/utils.ori", "line": 8, "expression": "helper(data)" }
  ],
...
# Add operation
ori edit add "type User" "verified: bool"

# Remove operation
ori edit remove "type User.legacy_id"

# Rename operation
ori edit rename @old_func @new_func

# Move operation
ori edit move @helper --to utils
```

### From JSON File

```bash
ori edit apply edits.json
```

### From Stdin

```bash
echo '{"op":"set","address":"@fetch.attempts","value":"5"}' | ori edit apply -
```

### Options

```bash
ori edit set @fetch.attempts 5 --dry-run    # Preview only
ori edit set @fetch.attempts 5 --no-format  # Skip auto-format
ori edit set @fetch.attempts 5 --force      # Skip validation
```
...
```bash
ori edit set @fetch.attempts 5 --dry-run
```
...
ori check src/ --json > errors.json
...
ori edit apply-json '{"op":"add","address":"@api.imports","value":"use std.json { parse }"}'
