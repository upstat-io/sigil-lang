...
Semantic addressing allows precise, targeted operations on code elements without regenerating entire files. Every structural element in Ori is addressable by name, enabling AI to make surgical edits.
...
**File resolution:**
```
@math.add           -> src/math.ori
@http.client.fetch  -> src/http/client.ori
type db.models.User -> src/db/models.ori
```
...
```json
{
  ...
  "address": "@helper",
  "matches": [
    { "file": "src/math.ori", "line": 15 },
    { "file": "src/utils.ori", "line": 8 }
  ],
...
```
@math.add           -> src/math.ori:12
@http.client.fetch  -> src/http/client.ori:45
type db.User        -> src/db.ori:8
$config.timeout     -> src/config.ori:3
```

### Listing Addresses

```bash
ori address list                # List all addresses in project
ori address list @math          # List addresses in math module
ori address list --type func    # List all function addresses
ori address list --type type    # List all type addresses
```
...
{
  "addresses": [
    { "address": "@math.add", "file": "src/math.ori", "line": 12, "kind": "function" },
    { "address": "@math.subtract", "file": "src/math.ori", "line": 18, "kind": "function" },
    { "address": "type math.Point", "file": "src/math.ori", "line": 5, "kind": "type" }
  ]
}
```
...
```
> :address @math.add
{ "file": "src/math.ori", "line": 12, "type": "(int, int) -> int" }
...
ori refactor extract-function @process.body[2:4] --name @helper
ori refactor move @helper --to utils
```
...
ori address list "@api.*"

# Check if address exists
ori address exists @fetch_data

# Get the value at an address
ori address get @fetch_data.attempts
```
...
```bash
ori address info @fetch_data --json
```
