...
This document covers Ori's approach to values, bindings, and immutability: no reassignment, shadowing allowed, destructuring patterns, and binding syntax.

---

## Overview

Ori enforces **value semantics** throughout the language:
...
In Ori, once a value is created, it cannot be modified:
...
In Ori, every `let x =` creates a **new binding**. There is no way to modify an existing binding:
...
**In Ori, shadowing makes data flow explicit:**
```ori
...
**Key insight:** There is no syntax for mutation in Ori. Every `let x =` inside `run` creates a new binding. This eliminates an entire class of bugs where AI (or humans) forget whether a function mutates its argument.
...
Ori restricts where variable bindings can occur:
...
Ori uses `let` for all bindings inside patterns:
...
```ori
// Ori - shadowing makes this clean
@process (data: Data) -> Data = run(
    let data = step1(data),
    let data = step2(data),
    let data = step3(data),
