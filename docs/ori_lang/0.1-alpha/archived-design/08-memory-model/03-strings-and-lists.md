...
This document covers Ori's handling of strings and lists: Small String Optimization (SSO), structural sharing for lists, and the distinction between value and reference types.

---

## Overview

Ori optimizes strings and lists for immutable, functional programming:
...
All strings in Ori are immutable:
...
All lists in Ori are immutable:
...
When creating new lists from existing ones, Ori shares unchanged portions:
...
Ori lists use a persistent vector structure (similar to Clojure):
...
Ori automatically chooses value or reference semantics:
