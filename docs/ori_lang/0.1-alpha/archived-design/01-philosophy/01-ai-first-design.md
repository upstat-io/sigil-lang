...
Ori is designed with AI-authored code as the primary optimization target. This document explains why and how this shapes the language.
...
Ori is designed for AI as a first-class citizen, while remaining human-readable and writable. This isn't about making the language "AI-friendly" as an afterthought—it's about fundamentally rethinking what a programming language should optimize for.
...
| Concern | Human-First Languages | AI-First (Ori) |
|---------|----------------------|------------------|
...
Ori optimizes for these concerns.
...
Ori avoids "magic" features that work through hidden mechanisms:
...
Traditional editing requires regenerating entire files. Ori enables targeted edits:
...
```
// 22 characters (positional, not valid Ori)
fold(items, 0, +)
```

```ori
// 56 characters (named, valid Ori)
fold(
    .over: items,
    .initial: 0,
    .operation: +,
...
// Ori: Types don't have subtype relationships.
...
Ori doesn't make humans write code that's better for AI—it makes AI write code that's better for humans to read and maintain.
