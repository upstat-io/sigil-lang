...
A quick reference for all Ori patterns with syntax and examples.
...
- Binding: `value` (binds value to name)
- Wildcard: `_` (matches anything)
- Variant: `Some(value)`, `None`
- Struct: `{ x, y }`, `{ x, .. }`
- List: `[first, ..rest]`
- Or: `A | B`
- Guard: `value if value > 0`
...
- `.op` — Binary operation `(accumulator, item) -> new_accumulator`
...
- `.parallel` — Parallelize when number > threshold (optional)
