...
This document covers Ori's `try` pattern for error propagation. The `try` pattern provides explicit, scoped error handling that makes error flow visible and predictable.

---

## Philosophy

Many languages use a single-character operator (`?` in Rust, `?` in Swift) for error propagation. Ori takes a different approach:
...
The `try` pattern makes error propagation **visible** and **scoped**, following Ori's principle of explicit control flow.
...
2. If any expression returns `Err(error)`, immediately return `Err(error)`
...
```ori
try(
    // read_file returns Result<str, Error>
    let content = read_file(path),
...
try(
    let content = read_file(path),
    // len doesn't return Result, no propagation
...
try(
    let x = get_x(),
    let y = get_y(),
    // Final expression determines return type
    Ok(x + y),
)
```
...
@load (path: str) -> Result<Data, AppError> = try(
    ...
    let content = read_file(path).map_err(error -> AppError.Io(error))?,

    // parse returns Result<Data, ParseError>
    // Convert ParseError to AppError
    let data = parse(content).map_err(error -> AppError.Parse(error))?,
...
Combine with `?` to unwrap and propagate: `.map_err(error -> ...)?`
...
@process_file (path: str) -> Result<Output, AppError> = try(
    ...
    let content = read_file(path).map_err(error -> AppError.FileError(error))?,
    let config = parse_json(content).map_err(error -> AppError.ParseError(error))?,
...
In Ori, the `try` block explicitly shows where errors are handled.
...
Ori's `try` is similar to Haskell's do-notation for the `Either` monad, but with explicit syntax and error conversion.
...
@outer (path: str) -> Result<int, OuterError> = try(
    ...
    let data = try(
        ...
        Ok(validate(parsed)),
    ).map_err(error -> OuterError.from_inner(error))?,
...
@validate_all (items: [Item]) -> Result<[Item], [Error]> = run(
    let errors = filter_map(
        .over: items,
        .f: item -> validate(item).err(),
    ),
    if is_empty(.collection: errors) then Ok(items)
    else Err(errors),
)
```
...
// Avoid: relying on implicit conversion (Ori doesn't have this)
// Type error if errors don't match
let content = read_file(path)?
```
...
@load_config (path: str) -> Result<Config, ConfigError> = try(
    ...
    let config = parse(content).map_err(error -> ConfigError.ParseError(error))?,
    let validated = validate(config).map_err(error -> ConfigError.ValidationError(error))?,
    Ok(validated),
)
```
...
match(a(),
    Ok(first) -> match(b(first),
        Ok(second) -> match(c(second),
            Ok(third) -> Ok(third),
            Err(error) -> Err(error)),
        Err(error) -> Err(error)),
    Err(error) -> Err(error))
