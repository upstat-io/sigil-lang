...
This document covers Ori's panic mechanism: what panics are, when to use them, and how to test panic conditions.

---

## Philosophy

Ori distinguishes between two kinds of failures:
...
```
PANIC at src/main.ori:42:5
  in function: @get
  message: index out of bounds: got 10, length is 5
  stack trace:
    @get (src/main.ori:42)
    @process (src/main.ori:78)
    @main (src/main.ori:95)
```
...
Ori provides `assert_panics` to verify that code panics when it should:
...
When a panic occurs, Ori provides a full stack trace:

```
PANIC at src/processor.ori:156:9
  in function: @validate_input
  message: assertion failed: input.len() > 0

  stack trace:
    @validate_input (src/processor.ori:156)
    @process (src/processor.ori:89)
    @handle_request (src/server.ori:234)
    @main (src/main.ori:12)
...
ORI_BACKTRACE=1 ori run program.ori

# Minimal output (default in release)
ORI_BACKTRACE=0 ori run program.ori
```
