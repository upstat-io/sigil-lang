...
This document covers Ori's exhaustiveness checking: compiler enforcement of complete pattern matching, wildcard warnings, and suppression with `#[allow]`.
...
@process (result: Result<Option<int>, str>) -> int = match(result,
    Ok(Some(value)) -> value,
    Ok(None) -> 0,
    Err(_) -> -1,
)
...
```ori
@is_success (status: Status) -> bool = match(status,
    Completed -> true,
    // matches Pending, Running, Failed
    _ -> false,
)
```
...
@bad (status: Status) -> str = match(status,
    _ -> "unknown",
    // never reached
    Completed -> "done"
)
...
@is_active (status: Status) -> bool = match(status,
    Running -> true,
    // silently handles new Cancelled
    _ -> false,
)
```
...
```ori
#[allow(wildcard_enum)]
@is_running (status: Status) -> bool = match(status,
    Running -> true,
    // intentionally catch-all
    _ -> false,
)
```
...
```ori
#[allow(wildcard_enum)]
@status_handlers (status: Status) -> Handler = match(status,
...
```ori
@describe (flag: bool) -> str = match(flag,
    true -> "yes",
    false -> "no",
)
...
```ori
@both_true (left: bool, right: bool) -> bool = match((left, right),
    (true, true) -> true,
    (true, false) -> false,
    (false, true) -> false,
    (false, false) -> false,
...
@quadrant (point: Point) -> int = match(point,
    { x, y } if x >= 0 && y >= 0 -> 1,
    { x, y } if x < 0 && y >= 0 -> 2,
    { x, y } if x < 0 && y < 0 -> 3,
    _ -> 4,
...
error[E0401]: unreachable pattern
  |
5 | @bad (number: int) -> str = match(number,
6 |     _ -> "any",
7 |     0 -> "zero"
...
error[E0401]: unreachable pattern
  |
5 | @bad (status: Status) -> str = match(status,
...
error[E0401]: unreachable pattern
  |
5 | @bad (number: int) -> str = match(number,
6 |     value if value > 0 && value < 0 -> "impossible"
...
@handle (status: Status) -> str = match(status,
    Pending -> "waiting",
    Running -> "active",
    Completed -> "done",
...
ori build --deny wildcard-enum

# Suppress all wildcard warnings
ori build --allow wildcard-enum

# Show additional exhaustiveness info
ori build --explain-exhaustiveness
```
