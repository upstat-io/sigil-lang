...
This document introduces Ori's pattern system—declarative constructs that replace common boilerplate with guaranteed-correct implementations.
...
```
// Compact but ambiguous (not valid Ori)
fold(items, 0, +)
```

```ori
// Verbose but clear (valid Ori)
fold(
    .over: items,
    .initial: 0,
    .operation: +,
