# Proposal: Enhanced Parallel and Concurrency Support in Ori
...
This proposal outlines enhancements to Ori's concurrency model based on analysis of how other languages fail at parallelism and concurrency. The goal is to make Ori's concurrency support not just safe, but **impossible to misuse** through type-level enforcement of correctness.
...
**Ori's existing advantage:** No `Mutex` type, message-passing only. But we can do better.
...
**Ori's position:** We have the Async capability, which colors functions but provides more value (explicit dependencies, easy mocking) for the same propagation cost.
...
**Current state:** Ori has structured concurrency via `parallel`. Enhance with explicit nursery/task group concept:
...
| Feature | Go | Rust | Erlang | Swift | Ori (Current) | Ori (Proposed) |
|---------|-----|------|--------|-------|-----------------|------------------|
...
Pony's reference capabilities are powerful but complex. Should Ori:
...
IPC requires serialization. Should Ori:
...
**Recommendation:** Option A - Ori is pre-1.0, clean breaks are acceptable.
