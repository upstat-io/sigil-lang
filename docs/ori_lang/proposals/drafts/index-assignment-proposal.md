# Proposal: Index Assignment via Copy-on-Write Desugaring
...
This proposal introduces index assignment syntax (`list[i] = x`) as syntactic sugar for copy-on-write reassignment. The expression `list[i] = x` desugars to `list = list.updated(key: i, value: x)`, where `updated` returns a new collection with the specified element replaced. Under ARC with refcount == 1 (the common case for local mutable bindings), the "copy" is optimized to in-place mutation automatically.

**Key features:**
1. **`IndexSet` trait** with `@updated (self, key: Key, value: Value) -> Self` method
2. **Grammar extension** to allow index expressions on the left side of assignment
3. **Nested index assignment** (`list[i][j] = x`) via recursive desugaring
4. **Built-in implementations** for `[T]` and `{K: V}`
...
Index assignment can be expressed as reassignment without mutable references:
...
The trait is added to the prelude alongside `Index`.
...
### Desugaring: Field + Index Combination

```ori
state.items[i] = x
```

The binding root must be an identifier. `state.items` is not a binding — `state` is. The desugaring reconstructs the full path:

```ori
state = State { ...state, items: state.items.updated(key: i, value: x) }
```

This follows the same pattern as nested index assignment: the compiler identifies the root binding (`state`), then builds the reconstruction expression from the root down to the assignment target.

**This proposal does not require field assignment syntax** (`state.items = x`). It only introduces index assignment. The `state.items[i] = x` form is included because it is the natural composition of struct spread with index update, and refusing it would leave a confusing gap.
...
The left-hand side of an assignment can now be an identifier followed by any number of index and field access operations. The root must still be a bare identifier (a mutable binding in scope).
...
2. **Index trait required for reads**: Each `[key]` in the chain (except the last) must resolve via the `Index<Key, Value>` trait on the receiver type.

3. **IndexSet trait required for writes**: The final `[key]` in the chain must resolve via `IndexSet<Key, Value>` on the receiver type. The value type of the `Index` impl and the value type of the `IndexSet` impl must agree.

4. **Type of assigned value**: The right-hand side expression must be assignable to the `Value` type parameter of the `IndexSet` impl on the target.

5. **Self type returned**: `IndexSet.updated` returns `Self`, so the reassignment is always type-correct.
...
Add a section on index assignment describing the desugaring, mutability requirements, and interaction with ARC:

- Index assignment syntax and what it desugars to
- Requirement that the root binding be mutable
- Nested index assignment
- Field-then-index combinations

### `09-expressions.md`

Update the Index Trait section to cross-reference `IndexSet` and index assignment. Add examples of index assignment in the expressions chapter.
...
1. Extend the parser to accept `assignment_target` (identifier + index/field chains) on the left side of `=`
2. Emit a new AST node (or annotated assignment node) that carries the chain of index/field accesses

### Phase 3: Desugaring Pass

1. In the AST lowering or type checker, desugar `target[k] = v` into `target = target.updated(key: k, value: v)`
2. Handle nested cases and field-index combinations

### Phase 4: Type Checker Integration

1. Resolve `IndexSet` trait in the type checker
2. Validate mutability of root binding
3. Validate key and value types against `IndexSet` impl
4. Emit appropriate diagnostics for errors

### Phase 5: Tests

1. Spec tests in `tests/spec/` for all cases (simple, nested, field+index, maps, errors)
2. Rust unit tests for parser, desugaring, and type checker changes
3. Error case tests for immutable bindings, parameters, missing trait impls, type mismatches
...
**Deferred.** Lenses are more powerful but significantly more complex to implement and explain. Index assignment covers the 95% use case. A lens system could be layered on top in a future proposal.

---

## Open Questions

1. **Should `IndexSet` be in the prelude?** This proposal assumes yes, since `Index` is in the prelude and they are complementary. But if `IndexSet` is only useful via the sugar (users never call `.updated()` directly), it could live in `std.ops` instead.

2. **Compound assignment operators**: Should `list[i] += 1` be supported? This would desugar to `list = list.updated(key: i, value: list[i] + 1)`. This is a natural extension but adds complexity. It could be addressed in a follow-up proposal.

3. **Slice assignment**: Should `list[0..3] = [a, b, c]` be supported? This is significantly more complex (different lengths, different types) and should be a separate proposal if desired.

4. **Evaluation order for nested index reads**: In `list = list.updated(key: i, value: list[i].updated(key: j, value: x))`, `list[i]` is read before the outer `updated` is called. If `i` or `j` involve side effects, the evaluation order matters. This proposal follows left-to-right evaluation, consistent with Ori's general evaluation order.
...
- **New syntax**: `target[key] = value` was previously a parse error; it now has defined semantics
