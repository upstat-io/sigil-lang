...
**Created:** 2026-01-22
**Rejected:** 2026-01-25
**Reason:** Superseded by const-keyword-proposal.md — decided to replace `$` ori entirely with `const` keyword for familiarity.
...
The `$` ori and all syntax remains exactly the same:
...
**Rejected:** The `$` ori provides visual distinction at usage sites. When you see `$timeout` in code, you immediately know it's a module-level constant, not a local variable. This aligns with Ori's philosophy of explicit oris (`@` for functions, `$` for constants).
...
**Rejected:** Overly verbose. "Constant" is sufficient since the `$` ori already implies module-level scope.
