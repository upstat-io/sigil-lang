...
dogs: [Dog] = [dog1, dog2, dog3]  // only dogs

// With dyn: can mix any Named type
pets: [dyn Named] = [dog1, cat1, bird1]  // mixed types
```
...
@process<T> (x: T) -> void where T: Processable = ...

// Dynamic: single function, smaller binary
@process (x: dyn Processable) -> void = ...
```
...
dog: Dog = ...
greet(dog)  // Dog automatically converted to dyn Named
```
...
```sigil
@process (pet: dyn Named) -> void = run(
    if is_type(pet, Dog) then
        dog = pet as Dog  // runtime checked, safe
        print(dog.breed)
)
```
...
trait Clone {
    @clone (self) -> Self  // can't use with dyn
}

// NOT object-safe: generic method
trait Converter {
    @convert<T> (self) -> T  // can't use with dyn
}
```
...
items: [dyn Iterator]  // What is Item?

// Must specify the associated type
items: [dyn Iterator<Item = int>]  // OK
```
...
@describe (x: dyn Named + Speaks) -> str =
    x.name() + " says " + x.speak()
```
