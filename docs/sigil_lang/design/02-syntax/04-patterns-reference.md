...
**Positional:**
```sigil
fold(collection, initial_value, operation)
```
...
@concat (strs: [str]) -> str = fold(strs, "", +)
@max (arr: [int]) -> int = fold(arr, arr[0], (a, b) -> if a > b then a else b)
```
...
**Examples:**
```sigil
@double_all (arr: [int]) -> [int] = map(arr, x -> x * 2)
@names (users: [User]) -> [str] = map(users, u -> u.name)
@lengths (strs: [str]) -> [int] = map(strs, s -> s.len())
```
...
@adults (users: [User]) -> [User] = filter(users, u -> u.age >= 18)
@non_empty (strs: [str]) -> [str] = filter(strs, s -> s.len() > 0)
```
...
```sigil
try(
    binding1 = result_expr1,  // must return Result
    binding2 = result_expr2,
    Ok(final_value)
)
```
...
| Property | Type | Description |
|----------|------|-------------|
| `.op` | `T` | Operation to timeout |
| `.after` | `Duration` | Timeout duration |
| `.on_timeout` | `T` | Value on timeout |

**Example:**
```sigil
@fetch_with_timeout (url: str) -> Result<Data, Error> = timeout(
    .op: http_get(url),
    .after: 30s,
