...
Patterns use **named properties** exclusively. This enforces Sigil's "explicit over implicit" philosophy:
...
1. **Explicit** — Property names make intent clear
2. **Consistent** — One way to write patterns (no style debates)
3. **AI-friendly** — AI learns one pattern, not multiple syntaxes
4. **Self-documenting** — Code explains itself without comments
...
retry(.op: fetch(), .attempts: 3, .backoff: exponential(...))
