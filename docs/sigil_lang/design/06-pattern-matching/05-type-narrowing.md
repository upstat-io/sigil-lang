...
@describe (s: Status) -> str = run(
    if is_variant(s, Running) then
        // s is Running(int)
        progress = as_variant(s, Running)
        "at " + str(progress) + "%"
    else if is_variant(s, Failed) then
        // s is Failed(str)
        msg = as_variant(s, Failed)
        "error: " + msg
    else
...
@area (s: Shape) -> float = run(
    if is_variant(s, Circle) then
        r = as_variant(s, Circle)
        3.14159 * r * r
    else if is_variant(s, Rectangle) then
        (w, h) = as_variant(s, Rectangle)
        w * h
    else
        // s must be Triangle
        (a, b, c) = as_variant(s, Triangle)
        // Heron's formula
        sp = (a + b + c) / 2.0
...
@describe_if (s: Status) -> str = run(
    ...
    else if is_variant(s, Running) then
        p = as_variant(s, Running)
        "at " + str(p) + "%"
...
@process (opt: Option<int>) -> int = run(
    ...
    check = is_some(opt) && some_other_condition(),
...
@extract (c: Container) -> int = run(
    if is_some(c.value) then
        inner = unwrap(c.value),
        if is_ok(inner) then
            unwrap(inner)
        else
            0
...
@process (opt: Option<int>) -> int = run(
    if opt.is_some() then run(
        opt = get_new_option(),  // shadows with new value
...
The shadow creates a new binding. The type checker cannot assume the new `opt` has the same narrowed type as the original.
