...
```sigil
// Good
data = parallel(.a: task_a(), .b: task_b()).await

// Bad: parallel without await is useless
parallel(.a: task_a(), .b: task_b())  // ERROR: async not awaited
```
...
5  |     spawn(background_task())
   ...
   = help: parallel(.main: main_work(), .background: background_task()).await
```
...
error[E0201]: async value not awaited
  --> src/main.si:8:5
   |
8  |     parallel(.a: task_a(), .b: task_b())
   |     ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ this parallel is never awaited
   |
   = note: add `.await` to wait for tasks: `parallel(...).await`
```
