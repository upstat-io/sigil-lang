...
@distribute_work (items: [Item]) -> async [Result<ProcessedItem, Error>] = run(
    work_ch = Channel<Item>.new(buffer: 100),
    result_ch = Channel<Result<ProcessedItem, Error>>.new(buffer: 100),
...
@aggregate_sources (sources: [Source]) -> async [Data] = run(
    data_ch = Channel<Data>.new(buffer: 100),
...
@pipeline (input: [int]) -> async [int] = run(
    ch1 = Channel<int>.new(buffer: 10),
    ch2 = Channel<int>.new(buffer: 10),
    ch3 = Channel<int>.new(buffer: 10),
...
@count_items (items: [Item]) -> async int = run(
    count_ch = Channel<int>.new(buffer: 100),
...
@count_chunk_and_send (items: [Item], ch: Channel<int>) -> async void = run(
    count = filter(items, item -> item.is_valid()).len(),
    ch.send(count).await
)
```
...
@worker (ctx: Context, work: Channel<Job>, results: Channel<Result<JobResult, Error>>) -> async void = run(
    loop(
        ...
        match(timeout(.op: work.receive().await, .after: 100ms, .on_timeout: None),
            Some(job) -> results.send(process(job)).await,
            None -> continue  // no work, check cancellation again
        )
    )
)
```
