...
// Type: Option<T>.map(T -> U) -> Option<U>
```
...
@lookup_then_process (id: str) -> Option<Result<ProcessedUser, Error>> = run(
    user = find_user(id),
    user.and_then(u -> process(u))  // Option<Result<...>>, not Option<Option<...>>
)

// Type: Option<T>.and_then(T -> Option<U>) -> Option<U>
```
...
```sigil
primary.or(fallback)  // primary if Some, otherwise fallback

find_user(id).or(default_user)
```
...
@require_user (id: str) -> Result<User, str> =
    find_user(id).ok_or("user not found: " + id)
...
value = unwrap(Err("oops"))  // PANIC!

error = unwrap_err(Err("oops"))  // "oops"
error = unwrap_err(Ok(42))       // PANIC!
```
...
value = expect(result, "config file must exist")
```
...
Err("e").map(x -> x * 2)  // Err("e")

// Type: Result<T, E>.map(T -> U) -> Result<U, E>
```

#### `map_err` -- Transform Error

```sigil
Ok(5).map_err(e -> "Error: " + e)     // Ok(5)
Err("e").map_err(e -> "Error: " + e)  // Err("Error: e")

// Type: Result<T, E>.map_err(E -> F) -> Result<T, F>
```
...
@parse_and_validate (s: str) -> Result<int, str> =
    parse_int(s).and_then(n ->
        if n > 0 then Ok(n)
        else Err("must be positive")
    )

// Type: Result<T, E>.and_then(T -> Result<U, E>) -> Result<U, E>
```

#### `or_else` -- Error Recovery

```sigil
primary_source().or_else(e -> fallback_source())
...
```sigil
// Option to Result
opt.ok_or(default_error)
opt.ok_or_else(compute_error)
...
name = find_name(id) ?? "anonymous"
count = parse_int(s) ?? 0

// Overkill for simple defaults
name = match(find_name(id),
    Some(n) -> n,
    None -> "anonymous"
)
```
...
// Good: fluent style
result = find_user(id)
    .and_then(u -> load_profile(u.id))
    .map(p -> p.display_name)
    .ok()
    ?? "unknown"

// Verbose alternative
result = match(find_user(id),
    Some(user) -> match(load_profile(user.id),
