...
@process (path: str) -> Result<Data, Error> = try(
    content = read_file(path),     // If Err, return immediately
    parsed = parse(content),       // If Err, return immediately
    validated = validate(parsed),  // If Err, return immediately
...
```sigil
try(
    content = read_file(path),  // read_file returns Result<str, Error>
...
try(
    ...
    length = len(content),      // len doesn't return Result, no propagation
    ...
)
```
...
@load (path: str) -> Result<Data, AppError> = try(
    ...
    data = parse(content) | e -> AppError.Parse(e),
...
@process_file (path: str) -> Result<Output, AppError> = try(
    ...
    content = read_file(path) | e -> AppError.FileError(e),
    config = parse_json(content) | e -> AppError.ParseError(e),
...
```sigil
@process (path: str) -> Result<Data, Error> = try(
    content = read_file(path),     // Error handling visible
    parsed = parse(content),       // Error handling visible
    Ok(transform(parsed))
)
```
...
@flexible_read (path: str, required: bool) -> Result<str, Error> = try(
    ...
    if required then
        content = result | e -> e  // Propagate error
    else
        content = result ?? ""     // Use default
    ,
...
@validate_all (items: [Item]) -> Result<[Item], [Error]> = run(
    errors = filter_map(items, i -> validate(i).err()),
...
@load_config (path: str) -> Result<Config, ConfigError> = try(
    content = read_file(path) | _ -> ConfigError.NotFound(path),
    config = parse(content) | e -> ConfigError.ParseError(e),
