...
### Binding
A name associated with a value. In Sigil, bindings are immutable by default. Created with `=` inside `run` or function parameters.
...
### Try Pattern
Error propagation pattern that evaluates expressions in sequence, returning early on any `Err`.
