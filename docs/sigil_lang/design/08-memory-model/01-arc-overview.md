...
@process_data (input: str) -> Result<Data, Error> = run(
    data = parse(input),      // data created, refcount = 1
    validated = validate(data), // data still referenced
...
@example () -> int = run(
    x = create_value(),  // x has refcount 1
    y = x,               // refcount now 2
...
@safe_read (path: str) -> Result<str, Error> = try(
    handle = open(path),       // FileHandle created
    content = read_all(handle), // Used
    Ok(content)
...
@destruction_order () -> void = run(
    first = create_a(),   // Created 1st
    second = create_b(),  // Created 2nd
    third = create_c(),   // Created 3rd
...
list = [1, 2, 3]
list2 = list + [4]  // If 'list' unique, reuse allocation
```
