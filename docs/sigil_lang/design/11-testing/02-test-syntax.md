...
@get (items: [int], index: int) -> int =
    if index < 0 || index >= len(items) then panic("index out of bounds")
    else items[index]

@test_get_panics tests @get () -> void = run(
    assert_panics(get([], 0)),
    assert_panics(get([1, 2, 3], -1)),
    assert_panics(get([1, 2, 3], 5))
)
```
...
```sigil
assert_panics_with(expression, "expected message")
```

```sigil
@test_get_panic_message tests @get () -> void = run(
    assert_panics_with(get([], 0), "index out of bounds")
)
```
...
@test_double tests @double () -> void = assert_eq(double(5), 10)
```
...
@test_database tests @query () -> void = run(
    // Setup
    db = create_test_db(),
    insert(db, {id: 1, name: "Alice"}),
    insert(db, {id: 2, name: "Bob"}),

    // Test
    result = query(db, "SELECT * WHERE id = 1"),
...
@test_first tests @increment () -> void = run(
    increment(),
    assert_eq($counter, 1)  // Might fail if test_second runs first!
)

@test_second tests @increment () -> void = run(
    increment(),
    assert_eq($counter, 2)  // Order-dependent!
)
```
...
| Boolean assert | `assert(condition)` |
| Equality assert | `assert_eq(actual, expected)` |
| Inequality assert | `assert_ne(actual, unexpected)` |
| Option asserts | `assert_some(opt)`, `assert_none(opt)` |
| Result asserts | `assert_ok(res)`, `assert_err(res)` |
| Panic assert | `assert_panics(expr)` |
