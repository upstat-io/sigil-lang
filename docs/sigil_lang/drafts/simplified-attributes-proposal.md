...
Sigil currently uses Rust's attribute syntax `#[name(...)]`. While functional, this syntax has issues:

1. **Visual noise** — The brackets add clutter without semantic value
2. **Cryptic appearance** — `#[` looks like line noise to newcomers
3. **Inconsistent with Sigil's philosophy** — Other sigils (`@`, `$`) don't require brackets

### Why Attributes Don't Need Sigil-Style Discoverability

Sigil uses sigils (`@`, `$`) to enable fast discovery:

| Sigil | Purpose | Discovery Use Case |
|-------|---------|-------------------|
...
Sigil uses `#` for length inside index expressions:
...
| `#test` | Functions | Mark as test (alternative to `tests` syntax) |
...
3. Provide `sigil fmt` auto-migration
4. Remove bracket syntax in next minor version

### Editor Support (VSCode TextMate Grammar)

The VSCode syntax highlighting needs to be updated in `editors/vscode-sigil/syntaxes/sigil.tmLanguage.json`.
...
"attribute": {
    ...
    "end": "\\]",
    "beginCaptures": {
        "0": { "name": "punctuation.definition.attribute.sigil" }
    },
    "endCaptures": {
        "0": { "name": "punctuation.definition.attribute.sigil" }
    },
    "patterns": [
        { "include": "#string" },
        { "name": "entity.other.attribute-name.sigil", "match": "[a-zA-Z_][a-zA-Z0-9_]*" }
    ]
}
```
...
"attribute": {
    ...
    "end": "(\\))|(?=\\s*[^,\\s])",
    "beginCaptures": {
        "1": { "name": "punctuation.definition.attribute.sigil" },
        "2": { "name": "entity.other.attribute-name.sigil" },
        "3": { "name": "punctuation.brackets.round.sigil" }
    },
    "endCaptures": {
        "1": { "name": "punctuation.brackets.round.sigil" }
    },
    "patterns": [
        { "include": "#string" },
        { "name": "entity.name.type.sigil", "match": "\\b[A-Z][a-zA-Z0-9_]*\\b" },
        { "name": "variable.parameter.sigil", "match": "[a-zA-Z_][a-zA-Z0-9_]*(?=\\s*=)" },
        { "name": "keyword.operator.assignment.sigil", "match": "=" }
    ]
}
```
    ...
    "patterns": [
        {
            ...
            "endCaptures": {
                "0": { "name": "punctuation.brackets.round.sigil" }
            },
            "patterns": [
                { "include": "#string" },
                { "name": "entity.name.type.sigil", "match": "\\b[A-Z][a-zA-Z0-9_]*\\b" },
...
| Syntax | Issue |
|--------|-------|
| `@attr` | Conflicts with function sigil |
| `$attr` | Conflicts with config sigil |
...
2. Doesn't conflict with other sigils
...
4. **Aligns with Sigil philosophy** — Sigils for discoverability, clean syntax otherwise
