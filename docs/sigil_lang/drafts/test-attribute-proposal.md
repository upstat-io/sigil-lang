...
An attribute-based approach aligns Sigil with industry conventions.
...
The `sigil fmt` tool will automatically convert old syntax to new:
...
2. **Phase 2:** `sigil fmt` auto-converts old to new
...
**Real-world example (from try.si):**
```sigil
// Before
...
| Alignment | Unique to Sigil | Similar to Rust, Java |
...
Per the [simplified attributes proposal](simplified-attributes-proposal.md), Sigil uses `#name()` not `#[name()]`. The brackets add noise without value.

### Why Require Targets?

Sigil's philosophy is explicit testing. Every test must declare what it tests. This enables:
- Coverage analysis
- Dead code detection
...
The `@` sigil identifies functions. Using `#test(@foo)` is consistent with how functions are referenced elsewhere:
- `@foo()` — call function
