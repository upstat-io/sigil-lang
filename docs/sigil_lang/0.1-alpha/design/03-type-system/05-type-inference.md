...
@process (user: User) -> str = run(
    ...
    let parts = upper.split(" "),   // inferred: [str]
...
let result = identity(42)  // identity<int> inferred
let opt = Some("hello")    // Option<str> inferred
```
...
let names = ["a", "b", "c"]    // [str] inferred
let pairs = [(1, "a"), (2, "b")]  // [(int, str)] inferred
```
...
```sigil
@process () -> int = run(
    let x = compute(),      // x's type comes from compute()'s return type
    let y = x * 2,          // y: int (from x: int)
    y + 10,
)
```
...
// Or: provide context
let result: Option<str> = first([])  // OK: T inferred from expected type
```
