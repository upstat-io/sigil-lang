...
let y: float = float(x)  // OK: explicit

// No implicit trait object conversion
let dog: Dog = ...
let pet: dyn Named = dog  // ERROR (without context)

let pet: dyn Named = dog as dyn Named  // OK: explicit
```
...
@process (n: dyn Named) -> void = run(
    if is_type(n, Dog) then run(
        let dog = n as Dog,  // runtime checked
        print(dog.breed),
    ),
)
```
