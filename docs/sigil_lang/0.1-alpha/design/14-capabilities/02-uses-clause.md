...
```sigil
@get_user (id: str) -> Result<User, Error> uses Http = try(
    let json = Http.get("/users/" + id)?,  // Calls Http.get on provided impl
    Ok(parse(json)),
)
...
@process (ids: [str]) -> Result<[str], Error> uses Http =
    traverse(ids, id -> run(
        let data = Http.get("/items/" + id)?,  // Lambda uses Http from enclosing scope
        Ok(data),
    ))
```
...
@main () -> void =
    ...
    run(
        let config = load_config(),  // uses Env
