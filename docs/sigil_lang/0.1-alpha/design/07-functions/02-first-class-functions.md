...
@main () -> void = run(
    ...
    let result = double_then_add5(3),  // (3 * 2) + 5 = 11
)
```
...
@main () -> void = run(
    let doubled = apply_twice(double, 5),  // double(double(5)) = 20
    print(doubled),
)
```
...
@main () -> void = run(
    ...
    let result = sum_reducer([1, 2, 3, 4]),  // 10
    print(result),
)
```
...
@main () -> void = run(
    let f = double,      // reference, not call
    let result = f(5),   // now call via variable
)
```
...
@main () -> void = run(
    ...
    let processed = proc.transform(5),  // 10
    let valid = proc.validate(-1),      // false
)
```
...
@main () -> void = run(
    ...
    let results = apply_all(transforms, 3),  // [6, 9, -3]
    print(results),
)
```
...
@main () -> void = run(
    let f = double,
    let g = double,
    let result = f == g,   // ERROR: functions cannot be compared
)
```
...
@main () -> void = run(
    let t1 = NamedTransform { name: "double", transform: x -> x * 2 },
    let t2 = NamedTransform { name: "double", transform: x -> x * 2 },
    let same = t1.name == t2.name,   // Compare by name
)
```
...
@main () -> void = run(
    let op1 = Double,
    let op2 = Double,
    let same = op1 == op2,  // true - enums are comparable
...
@main () -> void = run(
    let lengths: [int] = apply_mapper(["a", "bb", "ccc"], s -> s.len()),
    print(lengths),  // [1, 2, 3]
)
```
...
@main () -> void = run(
    ...
    let g = x -> x * 2,        // (int) -> int when used with ints
...
@main () -> void = run(
    ...
    let long_names = filter(["a", "abc", "ab"], x -> x.len() > 1),
)
```
...
@main () -> void = run(
    ...
    let result = double_then_add(5),   // (5 * 2) + 1 = 11
)
```
...
@main () -> void = run(
    let result = pipe(5, double, add_one),  // (5 * 2) + 1 = 11
)
```
