...
@handle_request () -> Result<Data, Error> = try(
    let response = http.client.get(url)?,   // http get
    let data = database.get(id)?,            // db get
    Ok(data),
)
```
...
@main () -> void = run(
    let result = add(2, 3),  // uses local add, result = 6
    print(str(result)),
)
```
...
@process (x: float) -> str = run(
    let result = sqrt(abs(x)),
    let formatted = string.format("{:.2}", result),
...
pub @fetch_user (id: int) -> Result<User, ApiError> = try(
    let token = get_token()?,
    let url = join([$base_url, "/users/", str(id)]),
    let response = get(url, .headers: {"Authorization": token})?,
    let user = parse(response.body)?,
    Ok(user),
)
```
