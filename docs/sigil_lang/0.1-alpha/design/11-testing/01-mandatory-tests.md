...
@test_fib_base tests @fibonacci () -> void = run(
    assert_eq(
        .actual: fibonacci(.n: 0),
        .expected: 0,
    ),
    assert_eq(
        .actual: fibonacci(.n: 1),
        .expected: 1,
    ),
)
...
@test_fib_sequence tests @fibonacci () -> void = run(
    assert_eq(
        .actual: fibonacci(.n: 2),
        .expected: 1,
    ),
    assert_eq(
        .actual: fibonacci(.n: 3),
        .expected: 2,
    ),
    assert_eq(
        .actual: fibonacci(.n: 4),
        .expected: 3,
    ),
    assert_eq(
        .actual: fibonacci(.n: 5),
        .expected: 5,
    ),
    assert_eq(
        .actual: fibonacci(.n: 6),
        .expected: 8,
    ),
)
```
...
```sigil
// math.si
@helper (x: int) -> int = x * 2  // private (no pub)

pub @double_helper (x: int) -> int = helper(.x: x)
...
```sigil
// math.test.si
use math { helper }  // Can import private items for testing
...
@test_is_prime tests @is_prime () -> void = run(
    // Edge cases
    assert(.cond: !is_prime(.n: 0)),
    assert(.cond: !is_prime(.n: 1)),

    // Small primes
    assert(.cond: is_prime(.n: 2)),
    assert(.cond: is_prime(.n: 3)),
    assert(.cond: is_prime(.n: 5)),
    assert(.cond: is_prime(.n: 7)),

    // Non-primes
    assert(.cond: !is_prime(.n: 4)),
    assert(.cond: !is_prime(.n: 6)),
    assert(.cond: !is_prime(.n: 9)),

    // Larger cases
    assert(.cond: is_prime(.n: 97)),
    assert(.cond: !is_prime(.n: 100)),
)

@test_check_factors tests @check_factors () -> void = run(
    assert(.cond: check_factors(.n: 7, .i: 5)),
    assert(.cond: !check_factors(.n: 25, .i: 5)),
)
```
