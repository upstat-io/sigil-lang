...
@fetch_all (ids: [int]) -> [User] uses Http, Async = parallel(
...
@fetch_seq (ids: [int]) -> [User] uses Http, Async = run(
    let users = [],
    for id in ids do
        users = users + [fetch_user(id)?],
    users,
)

// Concurrent - all at once
@fetch_par (ids: [int]) -> [User] uses Http, Async = parallel(
    .tasks: map(ids, id -> fetch_user(id)),
)
```
