...
let name = "Alice"              // No heap allocation (5 bytes)
let message = "This is a longer message..."  // Heap allocated

// Lists: structural sharing
let list1 = [1, 2, 3]
let list2 = list1 + [4]         // Shares [1,2,3] with list1
```
...
@example () -> void = run(
    ...
    let modified = "h" + greeting.slice(1, len(greeting)),
    void,
)
```
...
@process (input: str) -> str = run(
    let trimmed = input.trim(),          // New string
    let lower = trimmed.lower(),         // New string
    let replaced = lower.replace("a", "b"), // New string
    replaced,
)
```
...
@example () -> void = run(
    let short = "hi",
    let short_copy = short,  // Copied (inline - cheap)

    let long = "This is a very long string that lives on the heap",
    let long_ref = long,     // Shared (refcount incremented)
...
@example () -> void = run(
    ...
    let updated = [99] + numbers.slice(1, len(numbers)),
...
```sigil
let list1 = [1, 2, 3, 4, 5]
let list2 = list1 + [6]      // list2 shares [1,2,3,4,5] with list1
```
...
@without_sharing (items: [int]) -> [int] = run(
    let items = items + [1],   // Copy all, add 1
    let items = items + [2],   // Copy all, add 2
    let items = items + [3],   // Copy all, add 3
    items,  // Total: 3 full copies
)

// With sharing: O(1) or O(log n) per operation
@with_sharing (items: [int]) -> [int] = run(
    let items = items + [1],   // Share original, add node
    let items = items + [2],   // Share previous, add node
    let items = items + [3],   // Share previous, add node
...
let mixed = [1, "two", 3.0]  // ERROR: lists are homogeneous
```

**Append and prepend:**
```sigil
@example (items: [int]) -> [int] = run(
    let with_end = items + [100],      // Append
    let with_start = [0] + items,      // Prepend
    let combined = [0] + items + [100], // Both
    combined,
)
```
...
@example () -> void = run(
    let p1 = { x: 10, y: 20 },
    let p2 = p1,              // Copied (cheap for 16 bytes)
...
@example () -> void = run(
    let list1 = [1, 2, 3, 4, 5],
    let list2 = list1,        // Shared (refcount = 2)
...
@example (data: LargeStruct) -> int = run(
    let copy = data,           // Unnecessary if only reading
...
**Example:**
```sigil
let short = "hi"        // 24 bytes total (inline)
let medium = "hello world"  // 24 bytes total (inline)
let long = "this is a longer string for demonstration"  // 24 + 42 + 8 = 74 bytes
```
...
```sigil
let base = collect(0..1000000, i -> i)  // ~1M elements
let derived = base + [1000000]           // Shares 99.99% with base
