...
@example () -> int = run(
    let x = 5,         // Binding #1
    let x = x + 1,     // Binding #2 (shadows #1) - this is valid!
    x,                 // Returns 6
)
```
...
@explicit () -> int = run(
    let input = 10,
    let doubled = input * 2,    // New binding, input unchanged
    let final = doubled + 1,    // New binding, doubled unchanged
    final,
)
```
...
```sigil
@transform (data: Data) -> Data = run(
    let data = step1(data),   // Shadows parameter 'data'
    let data = step2(data),   // Shadows previous 'data'
    let data = step3(data),   // Shadows again
...
@shadow_demo () -> int = run(
    ...
    let y = x,          // y references binding #1
    let x = x * 2,      // Binding #2: x = 10 (shadows #1)
    let z = x + y,      // z = 10 + 5 = 15
    z,                  // Returns 15
)
```
...
@process_user (user: User) -> User = run(
    let user = user.validate(),    // Still "user"
    let user = user.enrich(),      // Still "user"
    let user = user.normalize(),   // Still "user"
    user,
)
...
@same_scope () -> int = run(
    let x = 1,
    let x = 2,   // Shadows in same run block - OK
    x,
)
```
...
@nested_scope (x: int) -> int = run(
    let x = x * 2,        // Shadows parameter - OK
    let result = run(
        let x = x + 1,    // Shadows outer x - OK
        x * 10,
    ),
...
@allowed () -> int = run(
    let x = compute(),   // Binding in run - OK
    let y = transform(x),
    y,
)
...
@not_allowed () -> int =
    let x = 5,     // ERROR: binding outside pattern
    x + 1
```
...
@load (path: str) -> Result<Data, Error> = try(
    let content = read_file(path)?,     // Binding, propagates Err
    let parsed = parse(content)?,       // Binding, propagates Err
    Ok(transform(parsed)),
)
```
...
```sigil
run(
    let x = compute(),    // Binding with let
    let result = use(x),
    result,
)
```
...
```sigil
run(
    let x: int = compute(),           // Explicit type
    let y = compute(),                 // Inferred type
    let result: float = float(x + y),  // Explicit for clarity
    result,
)
```
...
@first_two (items: [int]) -> int = run(
    let [a, b, ..rest] = items,   // First two and rest
    a + b,
)
```
...
**Partial extraction:**
```sigil
let { field1, field2 } = struct_value   // Only extract what you need
```

**Renaming on extraction:**
```sigil
let { x: horizontal, y: vertical } = point   // Rename x to horizontal
```
...
**Fixed elements:**
```sigil
let [first, second, third] = list   // Exactly 3 elements required
```

**With rest pattern:**
```sigil
let [first, ..rest] = list          // First element, rest as list
let [first, second, ..rest] = list  // First two, rest as list
let [..init, last] = list           // All but last, then last
```

**Nested:**
```sigil
let [[a, b], [c, d]] = matrix   // 2x2 matrix extraction
```
...
@sum_positives_explicit (items: [int]) -> int = run(
    let positives = filter(items, n -> n > 0),
...
@convert (input: str) -> Output = run(
    let parsed = parse(input),        // Different type
    let validated = validate(parsed), // Different state
...
@compute (point: Point) -> float = run(
    let { x, y } = point,   // Destructure at top
    let squared = x * x + y * y,
    sqrt(float(squared)),
)
