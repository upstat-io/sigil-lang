...
@fetch_user_name (id: int) -> Result<str, Error> = try(
    ...
    let { name, .. } = response.body,  // destructure the body
    Ok(name),
)
```
...
@bad (items: [int]) -> int = run(
    let [x, ..rest] = items,  // what if items is empty?
    x,
)
