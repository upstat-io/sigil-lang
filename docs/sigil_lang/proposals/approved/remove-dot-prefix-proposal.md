...
No sigil needed. Position relative to colon determines role.
...
```bash
sigil migrate remove-dot-prefix
```
...
| Less "sigil-y" | `@` and `$` remain distinctive |
...
- `compiler/sigilc/src/formatter/` — Formatting rules
- `docs/sigil_lang/0.1-alpha/spec/` — Grammar and examples
- `CLAUDE.md` — Quick reference

### Migration Tool

```bash
sigil migrate remove-dot-prefix [--dry-run] [path]
```
