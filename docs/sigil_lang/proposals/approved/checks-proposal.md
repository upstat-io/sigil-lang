...
Currently in Sigil, these checks are ad-hoc:
...
- Already ubiquitous in Sigil code
...
Sigil's choice is consistent with its philosophy:
...
4. **Is purely Sigil** - Patterns + named properties, nothing foreign
5. **Solves real problems** - Clear preconditions, postconditions, better error messages

The design prioritizes consistency with Sigil's existing patterns over visibility in function signatures, a tradeoff appropriate for a language with mandatory testing and short function bodies.
