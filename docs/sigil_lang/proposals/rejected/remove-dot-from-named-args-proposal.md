...
### Sigil's Other Sigils Earn Their Place

| Sigil | Purpose | Justification |
|-------|---------|---------------|
...
The `@` and `$` sigils provide meaningful disambiguation. The `.` does not.
...
The dot is Sigil-specific friction with no offsetting benefit.
...
A `sigil fix` command or formatter update can handle this automatically.
...
| Less "distinctive" Sigil syntax | Other sigils (`@`, `$`) remain distinctive |
...
- `compiler/sigilc/src/parser/grammar/expr.rs` — Call argument parsing
- `compiler/sigilc/src/lexer.rs` — No changes expected
- `docs/sigil_lang/0.1-alpha/spec/` — Update grammar and examples
- `docs/sigil_lang/0.1-alpha/design/` — Update syntax documentation
- `CLAUDE.md` — Update quick reference

### Migration Tool

Add `sigil migrate remove-dot-args` command to automatically update existing code.
...
### 3. Consistent Sigil Philosophy

Sigil uses sigils to mark different kinds of identifiers:
...
Each sigil explicitly identifies what kind of thing follows. Removing the dot breaks this consistency.
...
The dot is not redundant — it disambiguates parameter names from values and maintains Sigil's explicit, self-documenting philosophy. The minor reduction in typing does not justify the loss of clarity.
