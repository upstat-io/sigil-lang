...
**In Sigil, this distinction is unnecessary.**

Sigil doesn't expose monomorphization vs dynamic dispatch as a user-facing choice. The compiler decides the dispatch mechanism. Users shouldn't need to think about vtables.
...
Every mainstream language except Rust uses the trait/interface name directly. Sigil should follow the common pattern.
...
Rust needs `dyn` because it exposes the dispatch choice to users. Sigil doesn't.

### Why Not `impl Trait` Syntax?

Rust uses `impl Trait` for existential types in return position. Sigil doesn't need this distinction:

```sigil
// Sigil: just use the trait name
@make_printable () -> Printable = ...
...
Sigil's approach is simpler. The compiler figures out boxing/allocation.

### Object Safety

Rust has complex "object safety" rules determining which traits can be `dyn`. Sigil should have similar rules but doesn't need to expose them via a keyword. If a trait isn't usable as a type, that's a compile error with a clear message.
