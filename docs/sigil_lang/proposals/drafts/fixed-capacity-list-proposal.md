...
Regular Sigil lists (`[T]`) are heap-allocated and grow dynamically. This is flexible but has costs:
...
Sigil doesn't use semicolons, so this syntax is unavailable and would be confusing. We need a different approach.
...
| Syntax | Problem |
|--------|---------|
| `[int; 10]` | Semicolons not used in Sigil |
...
| Aspect | C++26 `inplace_vector` | Sigil `[T, max N]` |
|--------|----------------------|-------------------|
