...
Sigil already uses named arguments, making the pipe target explicit:
...
Sigil uses named arguments. The placeholder makes it explicit where the value goes — no guessing.
...
| **`_`** | Pattern matching | Already means "placeholder" in Sigil |

`_` is already used in pattern matching as a wildcard/placeholder. Consistent meaning.

### Why Not Auto-Insert?

Some languages automatically insert the piped value as the first or last argument. Sigil requires explicit placement because:
...
The pipe operator enables readable, left-to-right data transformation chains while maintaining Sigil's explicit philosophy through required placeholders.
