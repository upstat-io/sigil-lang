...
Sigil already uses named arguments, making defaults natural:
...
Sigil allows this because named arguments make it unambiguous:
```sigil
@f (a: int = 1, b: int) -> int
...
Sigil's named arguments make defaults more powerful than in positional languages:
...
Default parameters reduce boilerplate, make APIs more ergonomic, and work naturally with Sigil's named argument system.
